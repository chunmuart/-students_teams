...
<style>
    .login-container {
		...
		background-image: linear-gradient(to right, #009999, #66CCCC);
		position: fixed;
		left: 0;
		top: 0;
		bottom: 0;
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						login(this.loginForm).then(res => {
							...
							// 显示前端捕获到的错误信息，便于定位问题
							const msg = (err && err.data && err.data.msg) || (err && err.message) || '登录失败';
					...
					if(resp.code == 0){
						...
						this.$confirm('注册成功, 立即登陆?', '提示', {
							...
							login({userName: this.usersForm.userName, passWord: this.usersForm.passWord}).then(res => {
								...
								const msg = (err && err.data && err.data.msg) || (err && err.message) || '自动登录失败';
								this.$message({ message: msg, type: 'error' });
							});
						});						
					}else{
